...
`PKTIPAM_REMOVE_EXISTING=1` (or `0`) to answer that prompt from a script;
...
encryption key), `pktipam.db` and its `-wal`/`-shm`, `logs/`, `backups/` and
