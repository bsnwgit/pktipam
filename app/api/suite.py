"""
app/api/suite.py — pktHub integration endpoints (inbound: pktHub calling into pktIPAM).

Token flow:
  1. pktIPAM generates a random suite_token on first call to GET /api/suite/token
...
log = logging.getLogger("pktipam.api.suite")
...
async def set_direct_access(request: Request):
    ...
    Body: {"locked": true|false, "hub_redirect_url": "https://hub/app/5"}
