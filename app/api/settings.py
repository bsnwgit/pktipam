...
log = logging.getLogger("pktipam.settings")
...
def _store_value(key: str, value: Any) -> Any:
    ...
    if key in _ENCRYPTED_KEYS and isinstance(value, str) and value:
        from app.ipam.collectors.crypto import encrypt_str
...
async def read_secret(db: aiosqlite.Connection, key: str) -> str:
    ...
    if not isinstance(stored, str) or not stored:
        return ""
    from app.ipam.collectors.crypto import decrypt_str
...
async def test_notification(
    ...
    TEST_RULE = "pktIPAM Test"
    TEST_MSG  = "pktIPAM test notification — your configuration is working correctly."
    TEST_SEV  = "info"
    ...
    try:
        if channel == "slack":
            ...
            import httpx
            payload = {"text": f":white_circle: *pktIPAM Test — {TEST_RULE}*\n{TEST_MSG}"}
            ...
            from_addr = await _get("notify_email_from")        or "pktipam@localhost"
            ...
            msg["Subject"] = f"[pktIPAM Test] {TEST_RULE}"
            msg["From"]    = from_addr
            msg["To"]      = ", ".join(to_addrs)
            msg.attach(MIMEText(f"pktIPAM Test Notification\n\n{TEST_MSG}", "plain"))
            ...
            payload = {
                ...
                "payload": {
                    "summary": f"[pktIPAM Test] {TEST_RULE}: {TEST_MSG}",
                    "severity": "info",
                    "source": "pktipam",
                },
            }
