...
router = APIRouter()
log = logging.getLogger("pktipam.ai")

SYSTEM_PROMPT = """You are an IP address management assistant integrated into pktIPAM, an
enterprise IPAM platform covering subnets, VLANs, DHCP leases, DNS records, routes, and
address-conflict detection across sites.

You will receive structured IPAM context (subnet summaries, lease data, DNS records, conflicts)
...
- Flag address conflicts, exhausted subnets, or stale DHCP leases you notice
- Suggest investigation or remediation steps when appropriate
...
- Only answer questions about pktIPAM itself: subnets, VLANs, DHCP leases, DNS records,
  routes, address-conflict detection, and this app's own settings/features. Nothing else,
...
- If a question falls outside that — general knowledge, other software, other pktApp suite
  ...
  redirect the user to pktIPAM's own functionality. Do not partially answer it first.
...
# Other apps in the pktApp suite — mentions of these are out of pktIPAM's scope.
...
def _scope_violation(question: str) -> str | None:
    ...
    if _INJECTION_RE.search(question):
        return (
            "I can only help with pktIPAM itself — subnets, DHCP/DNS, and address conflicts. "
            "I can't change roles or ignore my instructions."
        )
    m = _OTHER_APP_RE.search(question)
    if m:
        return (
            f"That looks like a question about {m.group(1)}, which is outside pktIPAM's scope. "
...
def _strip_leaked_prompt(answer: str) -> str:
    ...
    if marker in answer.lower():
        return (
            "I can't share my system instructions. Ask me something about pktIPAM's "
            "subnets, DHCP/DNS, or address conflicts instead."
        )
    return answer
...
async def chat(
    ...
    """Send a question + IPAM context to the active AI provider and return the answer."""
