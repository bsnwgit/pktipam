"""
SQLite async database engine for the pktIPAM app database
...
async def seed_admin() -> None:
    ...
    Reads the plain-text password from PKTIPAM_ADMIN_PASSWORD (set by
    ...
    async with aiosqlite.connect(DB_PATH) as conn:
        ...
        if not admin_password:
            print(
                "\nFATAL: No users exist and PKTIPAM_ADMIN_PASSWORD is not set.\n"
                "       Set PKTIPAM_ADMIN_PASSWORD to create the initial admin account.\n"
                "       Example: PKTIPAM_ADMIN_PASSWORD=changeme python3 -m app.main\n",
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        await conn.execute(
            ...
            ("admin", "admin@pktipam.local", hashed, "admin"),
        )
        await conn.commit()
