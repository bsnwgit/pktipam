"""
pktIPAM configuration.

Priority order (highest -> lowest):
  1. Environment variables  (PKTIPAM_*)
  2. config.yaml — found via $PKTIPAM_CONFIG, $PKTIPAM_INSTALL_DIR/config.yaml,
     ./config.yaml, or ~/.pktipam/config.yaml
...
`install_dir` — the directory install.sh (or $PKTIPAM_INSTALL_DIR) was
pointed at — so the app works the same whether it's installed at
/opt/pktipam, in-place in a repo checkout, or anywhere else. Override any
...
def _find_config_path() -> Optional[Path]:
    ...
    install_dir = os.environ.get("PKTIPAM_INSTALL_DIR")
    if install_dir:
        candidates.insert(0, Path(install_dir) / "config.yaml")
    candidates.append(Path.home() / ".pktipam" / "config.yaml")

    env_path = os.environ.get("PKTIPAM_CONFIG")
    if env_path:
...
def _default_install_dir(config_path: Optional[Path]) -> Path:
    ...
    env_dir = os.environ.get("PKTIPAM_INSTALL_DIR")
    if env_dir:
        return Path(env_dir)
...
class Settings(BaseSettings):
    ...
    port: int = Field(default=_yaml_cfg.get("port", 8761))
    ...
    # Set by install.sh from PKTIPAM_ADMIN_PASSWORD; ignored if DB already has users.
    admin_password: str = Field(default="")

    # -- App database (SQLite) ------------------------------------------------
    db_path: str = Field(
        default=_yaml_cfg.get("db_path", str(_INSTALL_DIR / "pktipam.db"))
    )
    ...
    log_file: str = Field(
        default=_yaml_cfg.get("log_file", str(_INSTALL_DIR / "logs" / "pktipam.log"))
    )
...
def _validate_secrets(s: "Settings") -> None:
    ...
    if (s.secret_key or "").strip() in _INSECURE_SECRET_KEY_VALUES:
        raise RuntimeError(
            "pktipam refuses to start: secret_key is missing or still set to a "
    ...
    if (s.credential_key or "").strip() in _INSECURE_CREDENTIAL_KEY_VALUES:
        raise RuntimeError(
            "pktipam refuses to start: credential_key is missing or still set to "
