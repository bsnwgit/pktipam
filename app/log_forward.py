...
pktipam's own log records are written to the local app_logs table for the
...
log = logging.getLogger("pktipam.log_forward")
...
class SyslogForwardHandler(logging.Handler):
    ...
    def __init__(
        ...
        self._thread = threading.Thread(
            target=self._run, name="pktipam-log-forward", daemon=True
        )
        self._thread.start()
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        if record.name.startswith("pktipam.log_forward"):
            return
        try:
...
def send_test_message(host: str, port: int, protocol: str, app_name: str = "pktipam") -> dict:
    ...
    try:
        rec = logging.LogRecord(
            name="pktipam.test", level=logging.INFO, pathname=__file__, lineno=0,
            msg="pktipam log forwarding test message", args=(), exc_info=None,
        )
        h.emit(rec)
