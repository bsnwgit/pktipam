#!/bin/bash
# pktIPAM uninstall script — Ubuntu Server 22.04/24.04 LTS
...
# credential encryption key), pktipam.db and its -wal/-shm, logs, backups and
...
# WorkingDirectory, then $PKTIPAM_INSTALL_DIR, then /opt/pktipam.

set -euo pipefail

APP="pktipam"
DISPLAY="pktIPAM"
UNIT_PATH="/etc/systemd/system/pktipam.service"
...
# migration — plain *.db does not match "pktipam.db.pre-upgrade".
...
if [ "$PURGE" -eq 1 ]; then
    ...
    for entry in config.yaml pktipam.db logs backups ssl; do
        if [ -e "$INSTALL_DIR/$entry" ]; then
            echo "    $entry"
        fi
    done
