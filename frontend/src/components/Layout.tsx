...
export default function Layout({ children, chromeless = false }: { children: ReactNode; chromeless?: boolean }) {
  ...
  if (chromeless) {
    return (
      ...
      // itself with h-full, which collapses to zero against an auto-height
      // parent. This mirrors <main> below, so dropping the chrome changes what
      // is on screen and not how the page lays out.
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-12 flex-shrink-0 border-b border-gray-800 flex items-center px-4 md:px-6 gap-3 md:gap-5">
          ...
          <div className="flex items-center gap-1.5 text-sm ml-auto md:ml-0">
            ...
            <span className="text-white text-xs hidden sm:inline">IP Address Management</span>
          </div>
        </header>
