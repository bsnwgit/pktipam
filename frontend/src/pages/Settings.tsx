...
import { api, User, UserIn, Integration, IntegrationInput, SslStatus, UserApiKey, StorageStats, SnmpCredential } from '../api/client'
...
function LogForwardTester({ host, port, protocol }: { host: string; port: number; protocol: string }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      setOk(r.ok)
      setResult(r.ok
        ? `Sent 1 message to ${r.target} — check pktLog for "pktIPAM log forwarding test message"`
  ...
  return (
    <div>
      <div className="flex items-center gap-2">
        <TextInput value={value} onChange={onChange} placeholder={detected || 'https://pktipam.example.com'} mono />
        <button
          type="button"
...
// -- Suite token display (inbound — pktHub calling into pktIPAM) -----------------
...
// -- Sibling pkt app connections (outbound — pktIPAM calling into other pkt apps) --
// Named, reusable connections — add as many pktsnmp instances as you have
// deployments, each picked by name from a dropdown when setting up a
// pktsnmp_suite device collector under Collectors, instead of typing
// base_url/suite_token inline per collector. Same "define once, reuse
// everywhere" pattern as the SNMP Credentials tab above.
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-sm p-6" onClick={e => e.stopPropagation()}>
        <h2 className="text-lg font-semibold text-white mb-5">{editing ? `Edit — ${integration!.name}` : 'Add pktsnmp Connection'}</h2>
        <form onSubmit={submit} className="space-y-4">
          <div>
            ...
            <input value={form.name} onChange={e => setF('name', e.target.value)} required autoFocus
              placeholder="e.g. Main site pktsnmp" className={inp} />
          </div>
          <div>
            ...
            <input value={form.base_url} onChange={e => setF('base_url', e.target.value)} required
              placeholder="http://10.0.0.50:8767" className={inp} />
          </div>
          <div>
            ...
            <input type="password" value={form.suite_token} onChange={e => setF('suite_token', e.target.value)}
              required={!editing} placeholder="From that pktsnmp's Settings -> Security -> Suite Integration" className={inp} />
          </div>
...
function SiblingIntegrations() {
  ...
  return (
    <div className="space-y-3 py-3">
      ...
      {items.length === 0 && <p className="text-sm text-white py-2">No pktsnmp connections yet.</p>}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setConfirm(null)}>
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-white text-sm mb-5">
              Remove <strong className="text-white">{confirm.name}</strong>? Any device collector using it will
              lose its connection.
            </p>
...
// A credential is a named, reusable auth set (v2c community, or v3 user/
// auth/priv) — create it once here, then assign it to any number of
// device collectors under Collectors instead of re-entering SNMP auth
// per collector. Same pattern as pktsnmp's Settings -> SNMP Credentials.

interface CredentialFormState {
  name: string; description: string; snmp_version: 'v2c' | 'v3'; community: string
  security_name: string; security_level: 'noAuthNoPriv' | 'authNoPriv' | 'authPriv'
  auth_protocol: 'SHA' | 'MD5'; auth_key: string
  priv_protocol: 'AES' | 'DES'; priv_key: string
}

const EMPTY_CRED: CredentialFormState = {
  name: '', description: '', snmp_version: 'v2c', community: 'public',
  security_name: '', security_level: 'noAuthNoPriv', auth_protocol: 'SHA', auth_key: '',
  priv_protocol: 'AES', priv_key: '',
}

function CredentialFormModal({ cred, onClose, onSaved }: {
  cred: SnmpCredential | null; onClose: () => void; onSaved: () => void
}) {
  const editing = !!cred
  const [form, setForm] = useState<CredentialFormState>(
    editing ? {
      name: cred!.name, description: cred!.description, snmp_version: cred!.snmp_version,
      community: '', security_name: cred!.security_name, security_level: cred!.security_level,
      auth_protocol: cred!.auth_protocol, auth_key: '', priv_protocol: cred!.priv_protocol, priv_key: '',
    } : { ...EMPTY_CRED }
  )
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')
  const setF = <K extends keyof CredentialFormState>(k: K, v: CredentialFormState[K]) => setForm(f => ({ ...f, [k]: v }))
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError('')
    try {
      if (editing) await api.updateSnmpCredential(cred!.id, form)
      else await api.createSnmpCredential(form)
      onSaved()
    } catch (e: any) {
      setError(e.message ?? 'Save failed')
    } finally {
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={form.name} onChange={e => setF('name', e.target.value)} required
                placeholder="e.g. core-switches-v2c" className={inp} />
            </div>
            <div>
              <label className="text-xs text-white block mb-1">SNMP version</label>
              <select value={form.snmp_version} onChange={e => setF('snmp_version', e.target.value as 'v2c' | 'v3')} className={inp}>
          ...
          <div>
            ...
            <input value={form.description} onChange={e => setF('description', e.target.value)}
              placeholder="e.g. Read-only community for wired switches" className={inp} />
          </div>

          {form.snmp_version === 'v2c' && (
            <div>
              <label className="text-xs text-white block mb-1">Community string {editing ? '(leave blank to keep)' : '*'}</label>
              <input type="password" value={form.community} onChange={e => setF('community', e.target.value)}
                placeholder="public" required={!editing} className={`${inp} font-mono`} />
            </div>
          )}

          {form.snmp_version === 'v3' && (
            <div className="space-y-3 bg-gray-800/50 border border-gray-700 rounded-lg p-4">
              <p className="text-xs font-medium text-white">SNMPv3 parameters</p>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-white block mb-1">Security name</label>
                  <input value={form.security_name} onChange={e => setF('security_name', e.target.value)} className={inp} />
                </div>
                <div>
                  <label className="text-xs text-white block mb-1">Security level</label>
                  <select value={form.security_level} onChange={e => setF('security_level', e.target.value as CredentialFormState['security_level'])} className={inp}>
                ...
                {['authNoPriv', 'authPriv'].includes(form.security_level) && (
                  <>
                    <div>
                      ...
                      <select value={form.auth_protocol} onChange={e => setF('auth_protocol', e.target.value as 'SHA' | 'MD5')} className={inp}>
                        <option value="SHA">SHA</option>
                        <option value="MD5">MD5</option>
                      </select>
                    </div>
                    <div>
                      <label className="text-xs text-white block mb-1">Auth key {editing ? '(leave blank to keep)' : '*'}</label>
                      <input type="password" value={form.auth_key} onChange={e => setF('auth_key', e.target.value)}
                        required={!editing} className={inp} />
                    </div>
                  </>
                )}
                {form.security_level === 'authPriv' && (
                  <>
                    <div>
                      ...
                      <select value={form.priv_protocol} onChange={e => setF('priv_protocol', e.target.value as 'AES' | 'DES')} className={inp}>
                        <option value="AES">AES</option>
                        <option value="DES">DES</option>
                      </select>
                    </div>
                    <div>
                      <label className="text-xs text-white block mb-1">Privacy key {editing ? '(leave blank to keep)' : '*'}</label>
...
function SnmpCredentialsTab() {
  const [credentials, setCredentials] = useState<SnmpCredential[]>([])
  const [loading, setLoading] = useState(true)
  const [modal, setModal] = useState<'new' | SnmpCredential | null>(null)
  const [confirm, setConfirm] = useState<SnmpCredential | null>(null)
  const [error, setError] = useState('')

  const load = () => {
    setLoading(true)
    api.getSnmpCredentials().then(setCredentials).catch(e => setError(e.message)).finally(() => setLoading(false))
  }
  useEffect(load, [])

  const del = async (c: SnmpCredential) => {
    try {
      await api.deleteSnmpCredential(c.id)
      setConfirm(null)
      load()
    } catch (e: any) {
      setError(e.message)
  ...
  const versionBadge = (v: string) =>
    v === 'v3' ? 'bg-purple-900/40 text-purple-300 border border-purple-700/40' : 'bg-sky-900/40 text-sky-300 border border-sky-700/40'
  ...
  return (
    <div className="space-y-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <HelpButton title="SNMP Credentials — How It Works">
              <p>A credential is a <span className="text-gray-300 font-medium">named, reusable auth set</span> (v2c community string, or v3 user/auth/priv) — create it once here, then assign it to any number of device collectors under Collectors instead of re-entering SNMP auth per collector.</p>
              <p>Deleting a credential still assigned to a device collector is blocked — reassign that collector to a different credential first.</p>
              <p>Secret fields (community string, v3 auth/priv keys) are encrypted at rest and never sent back to the browser after saving — leave them blank when editing to keep the existing value.</p>
            </HelpButton>
          </div>
          <p className="text-xs text-white mt-0.5">Manage all SNMP auth here — assign to collectors under Collectors → Add Collector → Device</p>
        </div>
        <button onClick={() => setModal('new')}
          className="flex items-center gap-2 px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white text-sm rounded-lg transition-colors">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800">
                <th className="px-5 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Name</th>
                <th className="px-5 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Version</th>
                <th className="px-5 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Details</th>
                <th className="px-5 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Description</th>
                <th></th>
              </tr>
            </thead>
              ...
              {credentials.map(c => (
                <tr key={c.id} className="hover:bg-gray-800/30">
                  ...
                  <td className="px-5 py-3 text-white text-xs font-mono">
                    {c.snmp_version === 'v2c' ? '••••••••' : `${c.security_name || '—'} / ${c.security_level}`}
                  </td>
                  <td className="px-5 py-3 text-white text-xs">{c.description || '—'}</td>
                  <td className="px-5 py-3 text-right">
                    <button onClick={() => setModal(c)} className="text-xs text-white hover:text-sky-400">Edit</button>
                    <button onClick={() => setConfirm(c)} className="text-xs text-white hover:text-red-400 ml-3">Delete</button>
                  </td>
                </tr>
              ))}
              {credentials.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-8 text-center text-white">No credentials defined yet.</td></tr>
              )}
            </tbody>
          </table>
        )}
      ...
      {modal !== null && (
        <CredentialFormModal cred={modal === 'new' ? null : modal} onClose={() => setModal(null)}
          onSaved={() => { setModal(null); load() }} />
      )}
      ...
      {confirm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setConfirm(null)}>
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-6 max-w-sm w-full" onClick={e => e.stopPropagation()}>
            <h3 className="text-white font-semibold mb-2">Delete credential?</h3>
            <p className="text-white text-sm mb-5">
              Remove <strong className="text-white">{confirm.name}</strong>? Any device collector using it will
              lose its credential assignment.
            </p>
            <div className="flex justify-end gap-3">
                  ...
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-white">{u.auth_provider === 'saml' ? 'SSO' : 'local'}{u.username === me?.username && ' · you'}</p>
                      </div>
                    </div>
                  </td>
...
// -- Main page ---------------------------------------------------------------------
type TabId = 'general' | 'security' | 'data' | 'notifications' | 'resonance' | 'apikeys' | 'snmpcreds' | 'sites' | 'collectors' | 'system'
...
const APP_SECTION_LABEL = 'pktIPAM'
...
const OSS_NOTICES: Array<{ name: string; license: string }> = [
  ...
  { name: 'dnspython',          license: 'ISC' },
  { name: 'pywinrm',            license: 'MIT' },
...
// Storage + Backups, matching the golden layout's two Data sub-tabs. pktIPAM
// is SQLite-only (no ClickHouse/DuckDB option like pktsnmp/pktflow), so
// Storage has no backend picker — it shows retention settings, a manual
// cleanup action, and DB size/row-count stats instead.
...
export default function Settings() {
  ...
  const ALL_BUNDLE_FILES = ['pktipam.db', 'config.yaml']
  ...
  const runExport = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `pktipam-export-${new Date().toISOString().slice(0, 10)}.tar.gz`
      a.click()
  ...
  const loadStorageStats = () => {
    ...
    api.getStorageStats().then(setStorageStats).catch(() => {}).finally(() => setStatsLoading(false))
  }
  ...
  const runCleanupNow = async () => {
    ...
    try {
      const r = await api.runCleanupNow()
      setCleanupResult(`Removed ${r.alert_events_deleted} resolved alert(s), ${r.utilization_history_deleted} utilization-history row(s)`)
      loadStorageStats()
    } catch (e: any) {
  ...
  return (
    <>
      ...
      <h1 className="text-xl font-bold text-white">pktIPAM - Settings</h1>
      ...
      {tab === 'general' && (
        ...
        >
          <Field label="App name" hint="Displayed in browser tab and header">
            <TextInput value={str('app_name', 'pktIPAM')} onChange={v => set('app_name', v)} />
          </Field>
          ...
          <Field label="Base URL" hint="Used for SAML redirect URIs">
            <TextInput value={str('base_url')} onChange={v => set('base_url', v)} placeholder="http://SERVER-IP:8761" />
          </Field>
          <RestartServiceRow />
        </Section>
      )}
              ...
              <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-800 flex items-center gap-2">
                  ...
                  <HelpButton title="Suite Integration — How It Works">
                    ...
                    <p><span className="text-gray-300 font-medium">Sibling pkt apps</span> below is the other direction: pktIPAM calling into pktsnmp/pktflow/pktlog/pktpcap to reuse data they've already collected. Paste each app's own suite token (from that app's Settings → Security → Suite Integration) here.</p>
                  </HelpButton>
                </div>
      ...
      {dataTab === 'storage' && (
        <Section title="Storage" onSave={storageSave.save} saving={storageSave.saving} saved={storageSave.saved} error={storageSave.error}
          help={{
            ...
            content: <>
              <p>pktIPAM stores everything in a single SQLite database — there's no ClickHouse/DuckDB backend picker here like pktsnmp/pktflow, since nothing in pktIPAM needs a separate time-series store.</p>
              <p><span className="text-gray-300 font-medium">Retention</span> controls how long resolved alerts and subnet-utilization history rows are kept before the daily cleanup pass deletes them. Raw source data (DHCP leases, DNS records, ARP entries) isn't retention-limited — each collector poll replaces its own previous snapshot, so those tables never grow unbounded on their own.</p>
            </>,
          }}
        >
          <Field label="Alert event retention" hint="Days to keep resolved alert events before cleanup deletes them">
            <div className="flex items-center gap-3">
          ...
          <Field label="Utilization history retention" hint="Days to keep subnet-utilization trend history before cleanup deletes it">
            <div className="flex items-center gap-3">
              <NumberInput value={num('utilization_history_retention_days', 30)} onChange={v => set('utilization_history_retention_days', v)} min={1} max={3650} />
              <span className="text-sm text-white">days</span>
            </div>
          </Field>
          <Field label="Manual cleanup" hint="Run the retention cleanup immediately instead of waiting for the daily pass">
            <div className="space-y-2">
              <button onClick={runCleanupNow} disabled={cleanupRunning}
              ...
              {cleanupResult && (
                <p className={`text-xs ${cleanupResult.startsWith('Error') ? 'text-red-400' : 'text-green-400'}`}>{cleanupResult}</p>
              )}
            </div>
          </Field>
          <Field label="Database size" hint="Current on-disk size of the SQLite database file">
            {statsLoading ? (
              <p className="text-xs text-white">Loading…</p>
            ) : (
              <p className="text-sm text-white font-mono">
                {storageStats ? `${(storageStats.db_size_bytes / 1024 / 1024).toFixed(2)} MB` : '—'}
              </p>
            )}
          </Field>
          <Field label="Row counts" hint="Current row count per table">
            {statsLoading ? (
              <p className="text-xs text-white">Loading…</p>
            ) : storageStats ? (
              <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-xs text-white font-mono">
      ...
      {dataTab === 'logforward' && (
        <Section title="Log Forwarding" onSave={logForwardSave.save} saving={logForwardSave.saving} saved={logForwardSave.saved} error={logForwardSave.error}
          help={{
            ...
            content: <>
              <p>Ships pktIPAM's own application log to a syslog collector — normally <span className="text-gray-300 font-medium">pktLog</span>, which listens on port <code className="text-gray-400">5514</code> — so this app's logs sit alongside the rest of the estate instead of only in its local Logs page.</p>
              ...
              <p>Delivery is fire-and-forget on a background thread — if the collector is unreachable, lines are dropped and counted rather than blocking or crashing pktIPAM. Use <span className="text-gray-300 font-medium">Send test message</span> to confirm the path end to end.</p>
        ...
        >
          ...
          <Field label="Application name" hint="Appears as the APP-NAME field in the syslog message">
            <TextInput value={str('log_forward_app_name') || 'pktipam'} onChange={v => set('log_forward_app_name', v)} placeholder="pktipam" />
          </Field>
          <LogForwardTester
      ...
      {dataTab === 'backups' && (
        ...
        >
          ...
          <Field label="Export bundle" hint="Download pktipam.db + config.yaml as a .tar.gz">
            <div className="flex items-center gap-3 flex-wrap">
              {!exportPrompt ? (
          ...
          <Field label="Restore from bundle" hint="Upload a pktIPAM export .tar.gz to restore the database and config. Restart the service after restore.">
            <div className="space-y-3">
              <div className="flex items-center gap-3 flex-wrap">
        ...
        >
          ...
          {bool('notify_email_enabled') && (
            <>
              ...
              <Field label="From address"><TextInput value={str('notify_email_from')} onChange={v => set('notify_email_from', v)} placeholder="pktipam@yourorg.com" /></Field>
          ...
          {bool('notify_webhook_enabled') && (
            <>
              <Field label="URL">
                <TextInput value={str('notify_webhook_url')} onChange={v => set('notify_webhook_url', v)} placeholder="https://yourservice.com/pktipam-alert" mono />
              </Field>
              <Field label="Method">
      ...
      {tab === 'resonance' && (
        ...
        <Section title="Resonance" onSave={resonanceSave.save} saving={resonanceSave.saving} saved={resonanceSave.saved} error={resonanceSave.error}
          help={{
            ...
            content: <>
              <p>Resonance is the shared assistant surface for the pkt suite. It mounts as a launcher in the corner of every page, but the assistant itself runs on the resonance server rather than inside pktIPAM.</p>
              ...
              <p><span className="text-gray-300 font-medium">pktIPAM&rsquo;s own address</span> is what a browser types to reach this app, and it is the string that has to appear on the resonance key&rsquo;s origins list. Leave it blank and pktIPAM works it out from the request — correct for a direct install, and wrong behind a reverse proxy, where it sees the internal address rather than the one users type.</p>
              <p><span className="text-gray-300 font-medium">pktIPAM never sends your login credentials.</span> It vouches for whoever is signed in and receives a short-lived, single-use code the browser spends on opening the widget. The key below never reaches the browser.</p>
              <p><span className="text-gray-300 font-medium">The panel&rsquo;s insides belong to resonance.</span> It is an iframe served from resonance&rsquo;s own origin, so pktIPAM cannot restyle it or move its controls — where the buttons sit is a resonance change. What pktIPAM can do is make the panel bigger, with <span className="text-gray-300 font-medium">Panel size</span>, which is usually what &ldquo;more room to read&rdquo; actually needs.</p>
              ...
              <p><span className="text-gray-300 font-medium">The assistant can read this install&rsquo;s data.</span> The subnets, individual address records, VLANs, the conflicts pktIPAM has found, the collectors it polls, the estate summary, alert rules and the alerts they have fired, and pktIPAM&rsquo;s own diagnostic log. Each call is made by this page on the session of whoever is signed in, so it reaches only what that person could already open. The list is published at <code>/.well-known/resonance.json</code> and is fixed in the code rather than configurable — but it is inert unless <span className="text-gray-300 font-medium">Enabled</span> is on and the person&rsquo;s role is above <span className="text-gray-300 font-medium">No access</span> below.</p>
              <p><span className="text-amber-500 font-medium">No collector credential ever leaves through it</span> — the stored DHCP, DNS and SNMP configuration is not selected at all. Nothing it can call creates, edits or deletes a subnet, address, VLAN or collector, reserves or releases an address, or resolves a conflict.</p>
              ...
              <p><span className="text-gray-300 font-medium">A level never exceeds the role.</span> Two checks have to agree: the level set here, and pktIPAM&rsquo;s own rule for the thing being done. An analyst on <span className="text-gray-300 font-medium">Read and write</span> can acknowledge an alert, because analysts may; the same analyst cannot switch a rule, because that is an administrator&rsquo;s to do in the interface too.</p>
        ...
        >
          ...
          <Field label="pktIPAM's own address" hint="What browsers type to reach pktIPAM. Copy it onto the resonance key.">
          ...
          <Field label="What each role can do" hint="No access hides the launcher entirely. Read only lets the assistant look. Read and write also lets it act — never beyond what that role can already do in pktIPAM.">
            <div className="space-y-2">
        ...
        <div className="space-y-4">
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-800 grid grid-cols-3 gap-4 items-center">
              <h2 className="text-sm font-semibold text-white">System: {systemInfo?.app_name ?? 'pktIPAM'}</h2>
              <div className="col-span-2">
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            ...
            <div className="px-6 py-4">
              <p className="text-xs text-gray-400 mb-3">
                {systemInfo?.app_name ?? 'pktIPAM'} is built with the following open-source software:
              </p>
